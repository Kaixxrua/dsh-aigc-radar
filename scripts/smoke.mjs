/**
 * Smoke test: exercise the MCP client against the live AIGC Radar deployment.
 * Run with `npm run smoke` after `npm run build`.
 * Set AIGC_RADAR_MCP_TOKEN to smoke the authenticated (monthly-quota) path.
 */
...
const TOKEN = process.env.AIGC_RADAR_MCP_TOKEN ?? ''
const signal = AbortSignal.timeout(15000)

console.log(`mode: ${TOKEN ? 'authenticated' : 'anonymous (per-IP daily quota)'}`)
const search = await searchProjects(API_BASE, TOKEN, { q: 'mcp', sort: 'relevance' }, signal)
...
const categories = await listCategories(API_BASE, TOKEN, signal)
