...
export function applyRadarTools(ctx: Context, apiBase: string, mcpToken: string, timeoutMs: number, maxPageSize: number): void {
  ctx.systemPrompt.section({
    ...
    order: 110,
    text: 'Use the search_ai_projects tool whenever the user asks to find, compare, recommend, or reuse AI, Agent, MCP, RAG, or LLM open-source projects, repositories, SDKs, frameworks, components, templates, or examples — even if they do not name GitHub. Also run one proactive reuse check before implementing—or before recommending an architecture, technology, or build-vs-reuse approach for—a major module, complete capability, or independent subsystem, even without explicit discovery intent; design and selection advice is itself a reuse decision point: a major subsystem is a cohesive capability with its own architecture, integration, or maintenance boundary (auth, payments, workflow engines, queues, search/indexing, protocol implementations, end-to-end RAG or Agent subsystems), not merely a large diff. The goal is to surface mature, battle-tested technology before anyone rebuilds it from scratch. Skip the check for narrow work such as copy or styling, renames, localized bug fixes, tests/logging/types, validation, simple CRUD, or an isolated function, component, or endpoint change. The tool searches the curated AIGC Radar library: GitHub projects above a 500-star floor, enriched with categories and bilingual tags/descriptions. Translate non-English intent into concise English GitHub-style query terms while preserving product names. Run at most one search per unchanged requirement; inspect results before refining. The scope "today" is exploratory discovery of newly hot projects, not an official ranking. Call get_project_categories first only when the category taxonomy is unclear. Cite returned GitHub URLs as markdown links.',
  })
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      'Use it proactively: one check before implementing—or choosing the approach for—a major subsystem avoids rebuilding what already exists.',
    parameters: {
